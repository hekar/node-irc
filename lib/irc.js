...
class Client extends EventEmitter {
  constructor(server, nick, opt) { // eslint-disable-line no-unused-vars
    super();
    ...
    if (typeof arguments[2] === 'object') {
      const keys = Object.keys(this.opt);
    ...
    this.addListener('kick', (channel, who, by, reason) => { // eslint-disable-line no-unused-vars
      if (this.opt.autoRejoin) {
        this.send.apply(this, ['JOIN'].concat(channel.split(' ')));
      }
    });

    this.addListener('motd', (motd) => { // eslint-disable-line no-unused-vars
      this.opt.channels.forEach((channel) => {
        this.join(channel, _.noop);
      });
    });
  ...
  connect(retryCount, callback) {
    ...
    if (self.opt.secure) {
      ...
      if (typeof self.opt.secure === 'object') {
        ...
        for (let f in self.opt.secure) { // eslint-disable-line guard-for-in
    ...
    self.conn.cyclingPingTimer.on('pingTimeout', function() {
      self.connectionTimedOut(self.conn);
    });
    self.conn.cyclingPingTimer.on('wantPing', function() {
      self.connectionWantsPing(self.conn);
    });
  ...
  join(channel, callback) {
    // TODO: Figure out why the first character is <BOM>
    // HACK: Fix channel, so it does not start with <BOM>
    ...
    this.once('join' + channelName, () => {
      ...
      if (!_.includes(this.opt.channels, channel)) {
  ...
  part(channel, message, callback) {
    ...
    // upon reconnect
    if (this.opt.channels.indexOf(channel) < 0) {
